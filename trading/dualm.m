...
% month tracker
sd = datetime(datestr(position.dates(1)));
curr_month = month(sd);
...
for n=1:length(r_spy)
    ...
    % Current Day
    d=datetime(datestr(position.dates(n_date)));
    
    % Check trade at start of each month
    if month(d) ~= curr_month
        str = sprintf('Signal date: %s', d)
        ...
        if ~strcmp(position.ticker, curr_position.ticker)
            str = sprintf('%s: %s->%s, equity=$%.2f',d,curr_position.ticker, position.ticker, cash)
            ...
            % Signal generated by prior close. Start buying at next open.
